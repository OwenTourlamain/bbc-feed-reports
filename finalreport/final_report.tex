...
\section{Final System Overview}
  ...
  shown in \ref{fig:catalogue}. In this file is a single class, called database,
  ...
  The second file in the module, \textit{datatypes.py} as seen in
  \ref{fig:datatypes}, provides a collection of classes that represtend the
  ...
  separate file that can be loaded on start up, this schema can be seen in
  ...
  decided however that splitting development effort between two seperate
  interfaces would have negativley impacted the project and as such the mobile
  ...
  thre choice to use a template rendering engine as this allows for a different
  template to be rendered depending on various factors, such as if the user is
  accessing the site thorugh a mobile device.

%To-Do: Add UI screenshots
