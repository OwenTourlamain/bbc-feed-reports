...
\section{Aims and Objectives}
  ...
  digital news world. % Use \citep{} when referencing a paper indirectly, use \cite{} when you're mentioning the surname of the author in the text and you want only the date to be within parenthesis.
  ...
  increase user engagement on news platforms. % Removed should as it made you sound insecure, state don't doubt!
...
\section{Background and Rationale}
  ...
  use-case. % Check the BCU reference guide for the correct way of referencing websites and then check my dissertation to see how I went around the limitations of the BCU.bst file to reference particular things (music, videos, websites).
...
\section{Literature Review}
  ...
  of text to users, in the work of \cite{msnbc} we see a case study into how one large news
  ...
  swapped out" \citep{msnbc}. This closely mirrors the model proposed in this
  ...
  produced: % When you use \cite{} try to insert the author's surname in the flow of the sentence rather than just saying "in [1] we see that".
  ...
  % For block quotes, insert the reference in the bottom right inside the block quote (see my dissertation for an example).
  ...
  order to identify which metrics are important to track we can look at the published work of \cite{ux} and \cite{engagement}. \cite{ux} highlight that simple metrics,
  ...
  user experiences. The authors also found that the best way to measure user engagement
  ...
  indicator. % No need to re-cite the same work twice in two adjacent sentences.
  ...
  information'' \citep{newtarget} as there is always more content being added to
  ...
  later, this will be considered as a feature for this project. % Careful with "" orientation. Latex wants: `` and '' to open and close them correctly.
